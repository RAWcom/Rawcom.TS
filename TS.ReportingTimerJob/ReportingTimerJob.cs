...
namespace TS.ReportingTimerJob
{
    class ReportingTimerJob : SPJobDefinition
    {
        ...
        public ReportingTimerJob(SPWeb web)
            : base(string.Format("TS_Reporting Timer Job ({0})", web.Url), web.Site.WebApplication, null, SPJobLockType.Job)
        {
            Title = Name;
            WebUrl = web.Url;
        ...
        public override void Execute(Guid targetInstanceId)
        {
            ...
            SPSecurity.RunWithElevatedPrivileges(delegate()
            {
                try
                {
                    using (var site = new SPSite(SiteUrl))
                    {
                        using (var web = site.OpenWeb(WebUrl))
                        {
                            ...
                            ArrayList activeCustomers = GetActiveCustomers(hitDate);

                            if (activeCustomers != null && activeCustomers.Count > 0)
                            {
                                foreach (int custId in activeCustomers)
                                {
                                    CreateReportForCustomer(hitDate, custId);
                                }
                            }
                        }
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    ElasticEmailSendMailApp.ElasticTestMail.ReportErrorViaEmail(ex, WebUrl);

                }
            });
        }

        private ArrayList GetActiveCustomers(DateTime hitDate)
        {
        ...
        private void CreateReportForCustomer(DateTime hitDate, int customerId)
        {
            ...
            using (var site = new SPSite(SiteUrl))
            {
                using (var web = site.AllWebs[WebID])
                {
                    ...
                    if (targetList != null)
                    {
                        Customer customer = new Customer(customerId, SiteUrl, WebID);
...
___Rows___
        ...
        private static void UpdateReport(DateTime baseDate, DateTime hitDate, Customer customer, string subject, string bodyHTML, SPWeb web, Double totalHoursToReimburse)
        {
            ...
            if (targetList != null)
            {
                ...
                StartWorkflow(report, "Wysyłka Raportu");
    ...
    class Customer
    {
        ...
        public Customer(int customerId, string siteUrl, Guid webId)
        {
